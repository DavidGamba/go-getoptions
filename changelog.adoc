...
== WIP: v0.25.0-alpha: Major Refactor
...
----
+
WIP: The description could be called with opt.Description probably
...
* WIP: `NewCommand` signature change.
A command function is always required so make it explicit.
...
* WIP: `opt.SetRequireOrder` might be dropped since now we fully track subcommands.
Need to consider its usefulness when doing wrappers around other programs.

* WIP: `opt.CustomCompletion` might be dropped.
