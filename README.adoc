...
Commands are supported with built in utilities to make generating tools with commands easier.

A complete example can be found in link:./examples/mygit[]

A detailed explanation follows.


.examples/mygit/main.go
[source,go]
----
package main
...
When specific exit values are required, wrap your program:
...
This allows `defer` statements in `program()` to run while still being able to output an exit code.
